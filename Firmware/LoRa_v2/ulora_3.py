...
class uLoRa:
    ...
    def setup_lora(self):
        # Configuration code for LoRa module
        pass

    def send_data_str(self, data, length, frame_counter):
        # Assuming data is a string
        if isinstance(data, str):
            # Convert string to a format suitable for LoRa transmission (e.g., ASCII encoding)
            byte_data = data.encode('ascii')
        else:
            raise ValueError("Data must be a string")

        # Send the data over LoRa
        self._send(byte_data, length, frame_counter)

    def _send(self, data, length, frame_counter):
        # Implement the method to send data over LoRa
        # Example:
        self.cs.value(0)
        self.spi.write(data)
        self.cs.value(1)
